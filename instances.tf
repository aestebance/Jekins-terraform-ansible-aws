...
EOF
  ...
  provisioner "remote-exec" {
    when = destroy
    inline = [
      "MASTER_IP=$(cat /home/ec2-user/master_ip)",
      "java -jar /home/ec2-user/jenkins-cli.jar -auth @/home/ec2-user/jenkins_auth -s http://$MASTER_IP:8080 delete-node ${self.private_ip} > output.log 2>&1"
    ]
    connection {
      type        = "ssh"
      user        = "ec2-user"
